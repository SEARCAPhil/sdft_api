...
$parent_attachment = $Com->details($db,$id);

if(!isset($parent_attachment[0])) return 0;

//get parent basket
$parent=$attachments->details($db,$parent_attachment[0]->attachment_id);
...
if(in_array($__identity->uid,$collaborators_array)&&(@$parent[0]->status!='closed')){
	...
	$is_removed=$Com->remove($db,$id);

	if($is_removed){

		$activities->log_activity($db,$__identity->profile_id,$basket_id,'Removed a comment in '.$parent[0]->original_filename);
	}
