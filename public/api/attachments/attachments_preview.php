...
function download($db,$id){
	...
	try{
		...
		$returnFile='File not found!This will automatically close after 5 seconds.<script>setTimeout(function(){window.close();},5000);</script>';
          ...
          $server_file = $_SERVER['DOCUMENT_ROOT'].'/sdft_api/public/uploads/'.$basket_id.'/'.$row->filename;
          $server_extension = pathinfo($server_file, PATHINFO_EXTENSION);
					if(file_exists($server_file)){ 
            $file_exists = 1;
            //$returnFile.=readfile ($_SERVER['DOCUMENT_ROOT'].'/sdft_api/public/uploads/'.$basket_id.'/'.$row->filename);
            # view per mime
            if(in_array($server_extension,['png', 'jpg', 'jpeg', 'bmp'])) {
              $returnFile='<img src="../../../public/uploads/'.$basket_id.'/'.$row->filename.'"/>';
            }
            
            if($server_extension === 'pdf') {
              $returnFile =header('Content-type: application/pdf');
              $returnFile.=header('Content-Disposition: inline; filename="' . $filename . '"');
              $returnFile.=header('Content-Transfer-Encoding: binary');
              $returnFile.=header('Content-Length: ' . filesize($server_file));
              $returnFile.=header('Accept-Ranges: bytes');
              $returnFile.=ob_clean();
              $returnFile.=flush();
              $returnFile.=@readfile ($server_file);
              return $returnFile;
            }
          }
